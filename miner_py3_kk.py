...
def identifierConversion(expressionData,conversionTable="identifier_mappings.txt"):    
    idMap = pd.read_csv('identifier_mappings.txt',sep="\t")
...
def zerofilter(tpm):
    ...
    tmp_array_raw = np.array(tpm)
    rownames = tpm.index.values
    colname = tpm.columns.values[0]
    keep = []
    keepappend = keep.append
    for i in range(0,tmp_array_raw.shape[0]):
        if np.count_nonzero(tmp_array_raw[i,:]) >= round(float(tpm.shape[1])*0.5):
    ...
    tpm_zero_filtered = tmp_array_raw[keep,:]
    #print(tpm_zero_filtered.index.values)
    tpm_zero_filtered = pd.DataFrame(tpm_zero_filtered)
    tpm_zero_filtered.columns = list(tpm.columns)
    tpm_zero_filtered.index = list(np.array(tpm.index)[keep])
    return tpm_zero_filtered
    
def entropycheck(tpm):
    
    from scipy import stats
    
    tpm_array = np.array(tpm)

    tpm_entropy = []
    for i in range(0,tpm_array.shape[0]):
        tmp = entropy(tpm_array[i,:])
        tpm_entropy.append(tmp)

    tpmz_df = pd.DataFrame(tpm_array)
    tpmz_df.columns = list(tpm.columns)
    tpmz_df.index = list(np.array(tpm.index))
...
def biclusterMembershipDictionary(revisedClusters,background,label=2,p=0.05):
    ...
    if label == "excluded":
        ...
        print("Completed Bicluster Membership dictionary for excluded. done!")
        return members
    ...
    if label == "included":
        ...
        print("Completed Bicluster Membership dictionary for included. done!")
        return members
...
def mechanisticInference(axes,revisedClusters,expressionData,correlationThreshold=0.3,numCores=5,p=0.05,database="tfbsdb_tf_to_genes.pkl",dataFolder=os.path.join(os.getcwd(),"data"),override_database=None):
    import os
    
    print('Running mechanistic inference')
    
    if override_database is None:
        tfToGenesPath = os.path.join(database)
...
def generateInputForFIRM(revisedClusters,saveFile):
    ...
    identifier_mapping = pd.read_csv(os.path.join(os.getcwd(),"data","identifier_mappings.txt"),sep="\t")
...
def f1Decomposition(sampleMembers=None,thresholdSFM=0.333,sampleFrequencyMatrix=None):
    ...
    for iteration in range(1500):
        ...
        if members==predictedMembers:
            ...
            similarityMatrix = similarityMatrix.loc[predictedNonMembers,predictedNonMembers]
            probeSample = np.argmax(similarityMatrix.sum(axis=1))
            members = set(similarityMatrix.index[np.where(similarityMatrix.index[probeSample]==1)[0]])
            remainingMembers = predictedNonMembers
            nonMembers = remainingMembers-members
            f1 = 0
            continue
...
def causalNetworkAnalysis(regulon_matrix,expression_matrix,reference_matrix,mutation_matrix,resultsDirectory,minRegulons=1,significance_threshold=0.05,causalFolder="causal_results"):
    ...
    for mut_ix in range(mutation_matrix.shape[0]):
        ...
        causal_output.to_csv(os.path.join(causal_path,output_file).replace("\\","/"))
...
## Generate program activity
 ## transcriptional_programs - list defining the regulons for each program
## regulonModules - list defining genes for each regulon
## expressionData - table of  normalized rnaseq read counts 
## p - pvalue cutoff to indicate significance
## returnBkgd - if "yes" return gene activity matrix else if "no" return program activity
##
