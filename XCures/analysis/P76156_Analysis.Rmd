...
filename <- paste("output/",patient_id, "/", patient_id, "_regulon_drug_therapy_activity.csv",sep="")
        ...
        network_info <- network_info[1:20,]
...
### Drug Report Summary based on Patient's active risk-associated regulons.
```{r drug summary, results='asis', echo=F, warning=F, message=F}

for(target in unique(network_info$Target.Gene.s.)){
  target.drugs <- network_info %>%
    filter(Target.Gene.s. == target) %>%
    pull(Drug)
  
  cat(sep="","1. Target: **", target, "**  \n")
  cat(sep="","Drug(s): **", paste0(target.drugs, collapse=","), "**  \n")
  ...
  gbmtrials <- network_info %>%
   ...
   hallmarks.list <- network_info %>%
    filter(Target.Gene.s. == target) %>%
    select(Hallmarks) %>%
    unique() %>%
    pull()
   hallmarks.sep <- paste(str_split(hallmarks.list, pattern = ":")[[1]], collapse=", ")

   cat(sep = "","Evidence: **", activity.counts$Over,  "** risk-associated regulons containing the target(s), ", paste("**",paste(target.list, collapse=","),"**", sep=""), " of the drug(s) **",  paste0(target.drugs, collapse=","), "** are overactive in this patient with max RMST value of **", max(rmsts), "**.\n")
   cat(sep="", paste0(target.drugs, collapse=","), " has (**", paste0(gbmtrials,collapse= ","), "**) clinical trials for GBM and ", "is in ", anticancerPhaseIVs, "  \n" )
   if(hallmarks.sep != ""){
     cat(sep="", "+ Hallmark(s): ", hallmarks.sep, "  \n")
   }
   
   
   
     ppp <- plot_activity_histogram(target.drugs[1])
     print(ppp)
...
network_table1 <- network_info %>%
  ...
  dplyr::summarize(Drugs = paste(unique(Drug), collapse =", "),
            ...
            `GBM Trials Phase` = GBM_Trial_Phase,
