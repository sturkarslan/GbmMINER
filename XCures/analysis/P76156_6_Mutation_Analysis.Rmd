...
# patient CNV data
cnv_files <- dir(dna_dir, pattern = "*_cnvs.tsv",full.names = T,recursive = T)


# Get patients mutation data
patient_mutation_files <- grep(patient_id, mutation_files, value = T)
# Get patients CNV data
patient_cnv_files <- grep(patient_id, cnv_files, value = T)
...
patient_mutations_formatted <- patient_mutations %>%
  mutate(protein = paste0(GENE, ":",AAchange)) %>%
  mutate(sample = paste0("sample_", rownames(patient_mutations))) 

write.table(patient_mutations_formatted, file=paste0("data/", patient_id, "/", patient_id, "_CGI_input.txt"),row.names = F,col.names=T, sep = "\t")
...
cat(sep="", '<a name="cgi_mutations"></a>')
# load mutations
...
cat(sep="", '<a name="cgi_prescription"></a>')
...
if(length(cgi_prescription_results$DRUGS) != 0) {
  cat("\n\n")
