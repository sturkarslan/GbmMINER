...
```{r load_patients, echo=FALSE, message=FALSE,warning=FALSE}
...
```{r load_global_data, echo=FALSE, message=FALSE,warning=FALSE}
...
```{r patients_loop, results='asis', echo=F, warning=F, message=F}
...
dna_dir = fs::path("/Volumes/omics4tb2/SYGNAL/XCures/patients_processed_data",patient_id,"DNA")
# Get list of all mutation files from XCures folder
mutation_files1 <- fs::dir_ls(dna_dir, regexp =".annotated.txt" )
mutation_files2 <- fs::dir_ls(dna_dir, regexp ="_somatic.tsv" )
mutation_files <- c(mutation_files1,mutation_files2)
